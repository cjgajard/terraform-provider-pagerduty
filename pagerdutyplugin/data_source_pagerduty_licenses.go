		...
		Attributes: map[string]schema.Attribute{
			...
			"licenses": schema.ListNestedAttribute{
				...
				NestedObject: schema.NestedAttributeObject{
					Attributes: dataSourceLicenseAttributes,
				},
			},
		},
	}
...
func (d *dataSourceLicenses) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	err := retry.RetryContext(ctx, 2*time.Minute, func() *retry.RetryError {
		...
		model = flattenLicenses(list.Licenses, &resp.Diagnostics)
		return nil
	})
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("Error reading PagerDuty licenses"),
			err.Error(),
		)
	}
...
func flattenLicenses(licenses []pagerduty.License, diags *diag.Diagnostics) dataSourceLicensesModel {
	...
	return dataSourceLicensesModel{Licenses: types.ListValueMust(licenseObjectType, elements)}
}
